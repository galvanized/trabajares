#pragma config(Motor,  port1,           mL,           tmotorVex393_HBridge, openLoop)
#pragma config(Motor,  port2,           mC,           tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           mAL,           tmotorVex393_MC29, openLoop, encoderPort, None)
#pragma config(Motor,  port4,           mAR,          tmotorVex393_MC29, openLoop, encoderPort, None)
#pragma config(Motor,  port10,          mR,            tmotorVex393_HBridge, openLoop)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//
/*
       Y+
 LB     ^    LF
  1     |     0
        |
 X-<----+----> X+  FRONT
        |
        |
 RB     v     RF
  2     Y-     3


    Platform Type: VEX 2.0 Cortex
    Natural Language PLTW


Team 3170A
MIT License
*/

// This code is for the VEX cortex platform
#pragma platform(VEX2)

// Select Download method as "competition"
#pragma competitionControl(Competition)

//Main competition background code...do not modify!
#include "Vex_Competition_Includes.c"

byte armUpPow = 127;
byte armDownPow = -128;
byte armNeutralPow = 0;

byte clawOpenPow = 127;
byte clawClosePow = -127;
byte clawNeutralPow = 0;

byte winchPow = 127;

bool arm_triggered = false;

byte axisScale(byte in){ // provides deadzone functionality
        byte threshold = 20; // deadzone
        if(abs(in)<threshold){
            return 0;
        }
        else{
        	return in;
        }
}

byte capSum(byte x, byte y){
    // add up three bytes, returning a byte
    // prevents overflows
    short sum = (short) x + (short) y;

    if( sum > 127 ) sum = 127;
    else if( sum < -128 ) sum = -128;

    return (byte) sum;
}

void skidDrive(byte x, byte r){

    motor[mL] = capSum(x, r);
    motor[mR] = capSum(x, -r);

}


void setArm(byte power){
    //Set arm power. Up is positive.
    motor[mAL] = power;
    motor[mAR] = power;
}

void rcControl(){
    /* allow human control of the arm and pinchers
                            Control Setup

        left                                            right
        5U - Claw Open                      6U - Arm Up
        5D - Claw Close                     6D - Arm Down

    */
    skidDrive(axisScale(vexRT[Ch3]), axisScale(vexRT[Ch4]))
    //skidDrive(axisScale(vexRT[Ch3]), axisScale(vexRT[Ch4]), axisScale(vexRT[Ch1]));

    if(vexRT[Btn6U]) setArm(armUpPow);
    else{
        if(vexRT[Btn6D]) setArm(armDownPow);
        else setArm(armNeutralPow);
    }

    if(vexRT[Btn5U]) motor[mC] = clawOpenPow;
    else{
        if(vexRT[Btn5D]) motor[mC] = clawClosePow;
        else motor[mC] = clawNeutralPow;
    }

    if(vexRT[Btn8U]) motor[mW] = winchPow;
    else{
        if(vexRT[Btn8D]) motor[mW] = -winchPow;
        else motor[mW] = 0;
    }

}

void armUp(){
	motor[mC] = 80;
	delay(1000);
	motor[mC] = 0;
	setArm(127);
	delay(1100);
	setArm(0);
}


void autonomousUser(){
	armUp();
	delay(1000);
	skidDrive(127,0,0);
	delay(2000);
	skidDrive(0,0,0);
	delay(2000);
	for(int i; i<4; i++){
			skidDrive(-127,0,0);
			delay(500);
			skidDrive(0,0,0);
			delay(100);
			skidDrive(127,0,0);
			delay(600);
			skidDrive(0,0,0);
			delay(100);
	}
	skidDrive(-50,0,0);
	delay(1000);
	skidDrive(0,0,0);
}

void manualAutonomous(){
	if(vexRT[Btn8R] && !arm_triggered){
		arm_triggered=true;
		autonomousUser();
	}
	if(vexRT[Btn8L]){
		arm_triggered=false;
	}
}

/*---------------------------------------------------------------------------*/
/*                          Pre-Autonomous Functions                         */
/*                                                                           */
/*  You may want to perform some actions before the competition starts.      */
/*  Do them in the following function.  You must return from this function   */
/*  or the autonomous and usercontrol tasks will not be started.  This       */
/*  function is only called once after the cortex has been powered on and    */
/*  not every time that the robot is disabled.                               */
/*---------------------------------------------------------------------------*/

void pre_auton()
{
  // Set bStopTasksBetweenModes to false if you want to keep user created tasks
  // running between Autonomous and Driver controlled modes. You will need to
  // manage all user created tasks if set to false.
  bStopTasksBetweenModes = true;
}

/*---------------------------------------------------------------------------*/
/*                                                                           */
/*                              Autonomous Task                              */
/*                                                                           */
/*---------------------------------------------------------------------------*/

task autonomous()
{
  autonomousUser();
}

/*---------------------------------------------------------------------------*/
/*                                                                           */
/*                              User Control Task                            */
/*                                                                           */
/*---------------------------------------------------------------------------*/

task usercontrol()
{
  while(true)
	{
		delay(10);
		rcControl();
		manualAutonomous();
	}
}
